...
export function PublicMoviesList() {
  ...
  useEffect(() => {
    ...
    if (ratingFilter !== "all")
      newActiveFilters.push(`Rating: ${ratingFilter}`);
    ...
    <div className="space-y-6">
          ...
          <div className="relative">
            ...
            {searchQuery && (
              <Button
                variant="ghost"
                size="icon"
                className="text-primary/70 hover:text-primary absolute right-2 top-1/2 h-6 w-6 -translate-y-1/2 rounded-full p-0"
      ...
      {!isLoading && (
        <div className="text-muted-foreground text-sm">
          ...
          {movies.filter((m) => m.status === "published").length} movies
        </div>
      )}
